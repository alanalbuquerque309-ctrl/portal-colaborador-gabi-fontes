...
import { mapaRetornoAusenciaVigente } from '@/lib/avaliacao-retorno-ausencia';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const retornosVigentes = await mapaRetornoAusenciaVigente(
      supabase,
      equipe.map((c) => c.id),
      dataRef
    );
    ...
    return NextResponse.json({
      ...
      equipe: equipe.map((c) => ({
        ...
        retorno_previsto_vigente: retornosVigentes.get(c.id) ?? null,
      })),
    });
  } catch (e) {
