...
import { marcarLicencaSemanaRede } from '@/lib/avaliacao-marcar-licenca-rede';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const resultado = await marcarLicencaSemanaRede(supabase, {
    ...
    return NextResponse.json(
      {
        ...
        mensagem: resultado.ja_estava
          ? `${resultado.colaborador_nome} já estava de licença/afastamento nesta semana.`
          : `${resultado.colaborador_nome} registrado(a) em licença — pendência resolvida nesta semana.`,
      },
      { headers: NO_STORE }
    );
  } catch (e) {
