...
function aplicarRespostaLista(
  data: {
    registros?: Registro[];
    resumo?: { negativas?: number; demais?: number };
  },
  setRegistros: (r: Registro[]) => void,
  setResumo: (r: { negativas: number; demais: number }) => void
) {
...
export default function TermometroEmocoesAdminPage() {
  ...
  const [dataCarregada, setDataCarregada] = useState<string | null>(null);
  ...
  const carregar = useCallback((dataAlvo?: string) => {

    const alvo = dataAlvo ?? dataRef;

    setLoading(true);

    setErro('');

    fetch(`/api/admin/emocional/registros?data=${encodeURIComponent(alvo)}`, {
      ...
      .then(async (r) => {
        ...
        aplicarRespostaLista(data, setRegistros, setResumo);

        setDataCarregada(alvo);
      ...
      .catch((e) => {
        ...
        setResumo({ negativas: 0, demais: 0 });
  ...
  useEffect(() => {
    ...
    const hoje = hojeInputValue();

    setDataRef(hoje);

    carregar(hoje);

    // eslint-disable-next-line react-hooks/exhaustive-deps -- carga inicial só do dia corrente
  ...
  const precisaBuscar = dataRef !== dataCarregada;
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div>
          ...
          <p className="text-sm text-coffee-100 mt-1">
            ...
            Hoje você vê todas as respostas do dia corrente. Para outra data, escolha e clique em Buscar.

            Alertas de reação negativa continuam no topo do admin a qualquer momento.
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-lg border border-dourado-200 bg-dourado-50 px-3 py-2 text-sm font-medium text-coffee-base hover:bg-dourado-100 disabled:opacity-50"

          >

            {precisaBuscar ? 'Buscar' : 'Atualizar'}
          ...
          {dataRef !== hoje && (
            ...
            <button
              ...
              className="rounded-lg border border-cream-300 bg-white px-3 py-2 text-sm text-coffee-base hover:bg-cream-50 disabled:opacity-50"
      ...
      {precisaBuscar && !loading && (

        <p className="text-sm text-coffee-100 rounded-xl border border-cream-200 bg-cream-50 px-4 py-3">

          Data alterada para <span className="font-medium capitalize">{dataFmt}</span>. Clique em{' '}

          <strong>Buscar</strong> para carregar as respostas.
      ...
      {!loading && !erro && dataCarregada && registros.length === 0 && (
      ...
      {!loading && !erro && dataCarregada && negativas.length > 0 && (
      ...
      {!loading && !erro && dataCarregada && demais.length > 0 && (
