 ...
 * - OU sessão do portal com colaborador que tem role socio, admin, master ou gerente
 */
...
/** Sessão do portal com role `master` (redefinir senha de colaboradores, etc.). */
...
export async function getAdminViewerContext(): Promise<AdminViewerContext | null> {
  ...
  try {
    ...
    const role = normalizePortalRole((data as { role?: string } | null)?.role);
    if (role !== 'socio' && role !== 'admin' && role !== 'master' && role !== 'gerente') return null;
