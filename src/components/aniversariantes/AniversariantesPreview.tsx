...
export function AniversariantesPreview() {
  ...
  return (
    <div className="space-y-3">
      {aniversariantes.slice(0, 5).map((a) => (
        <div
          key={a.id}
          className="rounded-xl border border-dourado-200 bg-cream-50 px-4 py-3 flex items-center gap-3"
        >
          {a.foto_url ? (
            <img
              src={a.foto_url}
              alt=""
              className="w-10 h-10 rounded-full object-cover border border-dourado-200"
            />
          ) : (
            <div className="w-10 h-10 rounded-full bg-dourado-100 flex items-center justify-center border border-dourado-200">
      ...
      {aniversariantes.length > 5 && (
        <p className="text-coffee-100 text-xs text-center">
          <a href="/portal/aniversariantes" className="text-dourado-base hover:underline">
