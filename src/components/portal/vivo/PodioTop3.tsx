...
import type { RankingAvaliacaoItem } from '@/components/mural/ranking-ui';
...
/** Pódio visual: ordem 2º · 1º · 3º */
export function PodioTop3({
  itens,
  modo,
}: {
  ...
  const colunas: (RankingAvaliacaoItem | undefined)[] = [segundo, primeiro, terceiro];
  ...
  return (
    <div className="flex items-end justify-center gap-2 sm:gap-4 pt-2 pb-1">
      {colunas.map((item, ix) => {
        ...
        const nota =
          modo === 'semanal' ? item.media.toFixed(2) : item.media.toFixed(2);
        const rotuloNota = modo === 'semanal' ? 'Nota' : 'Média';
        ...
        return (
          ...
          >
            ...
            <p className="text-xs font-bold text-dourado-700 mt-0.5">
              {rotuloNota} {nota}
            </p>
            {modo === 'mensal' && item.semanas_avaliadas > 0 && (
              <p className="text-[10px] text-cafeteria-600 mt-0.5 text-center leading-tight">
                {rotuloSemanasAvaliadas(item.semanas_avaliadas)}
              </p>
            )}
            <div
