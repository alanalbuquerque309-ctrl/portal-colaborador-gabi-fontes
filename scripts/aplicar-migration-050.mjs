/**
 * Aplica supabase/migrations/050_seguranca_recuperacao_e_rate_limit.sql
 * Uso: npm run db:apply-050
 */
import fs from 'fs';
...
const sqlPath = path.join(portalRoot, 'supabase', 'migrations', '050_seguranca_recuperacao_e_rate_limit.sql');
const sql = fs.readFileSync(sqlPath, 'utf8');

const client = postgres(databaseUrl, { max: 1, ssl: 'require' });
try {
  await client.unsafe(sql);
  console.log('Migration 050 aplicada.');
} catch (e) {
