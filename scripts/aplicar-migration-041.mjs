/**
 * Aplica supabase/migrations/041_aniversario_dia_acao.sql
 * Uso: npm run db:apply-041
 */
import fs from 'fs';
...
const sqlPath = path.join(portalRoot, 'supabase', 'migrations', '041_aniversario_dia_acao.sql');
const sql = fs.readFileSync(sqlPath, 'utf8');

const client = postgres(databaseUrl, { max: 1, ssl: 'require' });
try {
  await client.unsafe(sql);
  console.log('Migration 041 aplicada.');
} catch (e) {
