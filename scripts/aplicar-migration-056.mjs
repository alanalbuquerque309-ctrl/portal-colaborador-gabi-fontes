/**
 * Aplica supabase/migrations/056_io_indexes.sql
 * Uso: npm run db:apply-056
 */
import fs from 'fs';
...
const sqlPath = path.join(portalRoot, 'supabase', 'migrations', '056_io_indexes.sql');
const sql = fs.readFileSync(sqlPath, 'utf8');

const client = postgres(databaseUrl, { max: 1, ssl: 'require' });
try {
  await client.unsafe(sql);
  console.log('[db:apply-056] OK — índices de I/O aplicados.');
} catch (e) {
  console.error('[db:apply-056] ERRO:', e instanceof Error ? e.message : e);
  process.exit(1);
} finally {
  await client.end();
}
