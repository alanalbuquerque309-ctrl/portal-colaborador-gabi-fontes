/**
 * Aplica supabase/migrations/051_seguranca_audit_log.sql
 * Uso: npm run db:apply-051
 */
import fs from 'fs';
...
const sqlPath = path.join(portalRoot, 'supabase', 'migrations', '051_seguranca_audit_log.sql');
const sql = fs.readFileSync(sqlPath, 'utf8');

const client = postgres(databaseUrl, { max: 1, ssl: 'require' });
try {
  await client.unsafe(sql);
  console.log('Migration 051 aplicada.');
} catch (e) {
