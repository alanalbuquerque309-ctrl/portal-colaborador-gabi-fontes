/**
 * Aplica supabase/migrations/052_cafe_conecta.sql
 * Uso: npm run db:apply-052
 */
import fs from 'fs';
...
const sqlPath = path.join(portalRoot, 'supabase', 'migrations', '052_cafe_conecta.sql');
const sql = fs.readFileSync(sqlPath, 'utf8');

const client = postgres(databaseUrl, { max: 1, ssl: 'require' });
try {
  await client.unsafe(sql);
  console.log('[db:apply-052] OK — tabelas Café Conecta aplicadas.');
} catch (e) {
  console.error('[db:apply-052] ERRO:', e instanceof Error ? e.message : e);
  process.exit(1);
} finally {
  await client.end();
}
